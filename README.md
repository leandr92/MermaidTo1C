...
MermaidTo1C/
├── src/                          # Исходные файлы
│   ├── index.js                 # Точка входа для сборки (импорт и инициализация Mermaid)
...
Собранный файл `mermaid_all_in_one.js` имеет размер примерно 6.5 МБ (несжатый). Это нормально, так как включает:
- Mermaid.js 11.12.2
- Все зависимости (D3, dagre, cytoscape и др.)
...
5. **Размер файла**: Собранный файл имеет размер около 6.5 МБ (несжатый). Это нормально, так как включает всю библиотеку Mermaid и все зависимости.
